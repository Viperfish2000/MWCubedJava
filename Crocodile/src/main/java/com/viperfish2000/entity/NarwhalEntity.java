...
public class NarwhalEntity extends AbstractGroupFishEntity {

	private static final DataParameter<BlockPos> TREASURE_POS = EntityDataManager.createKey(NarwhalEntity.class,
			DataSerializers.BLOCK_POS);
	private static final DataParameter<Boolean> GOT_FISH = EntityDataManager.createKey(NarwhalEntity.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> MOISTNESS = EntityDataManager.createKey(NarwhalEntity.class,
	...
	public NarwhalEntity(EntityType<? extends NarwhalEntity> type, World worldIN) {
		super(type, worldIN);
		this.moveController = new NarwhalEntity.MoveHelperController(this);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new NarwhalEntity.SwimToTreasureGoal(this));
		this.goalSelector.addGoal(2, new NarwhalEntity.SwimWithPlayerGoal(this, 4.0D));
		this.goalSelector.addGoal(4, new RandomSwimmingGoal(this, 1.0D, 10));
		this.goalSelector.addGoal(4, new LookRandomlyGoal(this));
		this.goalSelector.addGoal(5, new LookAtGoal(this, PlayerEntity.class, 6.0F));
	//	this.goalSelector.addGoal(5, new DolphinJumpGoal(this, 10));
		this.goalSelector.addGoal(6, new MeleeAttackGoal(this, (double) 1.2F, true));
		this.goalSelector.addGoal(8, new NarwhalEntity.PlayWithItemsGoal());
	...
	public static boolean func_223364_b(EntityType<NarwhalEntity> p_223364_0_, IWorld p_223364_1_, SpawnReason reason,
	...
	static class MoveHelperController extends MovementController {
		private final NarwhalEntity dolphin;

		public MoveHelperController(NarwhalEntity dolphinIn) {
			super(dolphinIn);
	...
	class PlayWithItemsGoal extends Goal {
		...
		public boolean shouldExecute() {
			if (this.field_205154_b > NarwhalEntity.this.ticksExisted) {
				return false;
			} else {
				List<ItemEntity> list = NarwhalEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
						NarwhalEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), NarwhalEntity.ITEM_SELECTOR);
				return !list.isEmpty()
						|| !NarwhalEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND).isEmpty();
			}
		}
		...
		public void startExecuting() {
			List<ItemEntity> list = NarwhalEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
					NarwhalEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), NarwhalEntity.ITEM_SELECTOR);
			if (!list.isEmpty()) {
				NarwhalEntity.this.getNavigator().tryMoveToEntityLiving(list.get(0), (double) 1.2F);
				NarwhalEntity.this.playSound(SoundEvents.ENTITY_DOLPHIN_PLAY, 1.0F, 1.0F);
			}
		...
		public void resetTask() {
			ItemStack itemstack = NarwhalEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND);
			if (!itemstack.isEmpty()) {
				this.func_220810_a(itemstack);
				NarwhalEntity.this.setItemStackToSlot(EquipmentSlotType.MAINHAND, ItemStack.EMPTY);
				this.field_205154_b = NarwhalEntity.this.ticksExisted + NarwhalEntity.this.rand.nextInt(100);
			}
		...
		private void func_220810_a(ItemStack p_220810_1_) {
			if (!p_220810_1_.isEmpty()) {
				double d0 = NarwhalEntity.this.getPosYEye() - (double) 0.3F;
				ItemEntity itementity = new ItemEntity(NarwhalEntity.this.world, NarwhalEntity.this.getPosX(), d0,
						NarwhalEntity.this.getPosZ(), p_220810_1_);
				itementity.setPickupDelay(40);
				itementity.setThrowerId(NarwhalEntity.this.getUniqueID());
				float f = 0.3F;
				float f1 = NarwhalEntity.this.rand.nextFloat() * ((float) Math.PI * 2F);
				float f2 = 0.02F * NarwhalEntity.this.rand.nextFloat();
				itementity.setMotion(
						(double) (0.3F * -MathHelper.sin(NarwhalEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(NarwhalEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.cos(f1) * f2),
						(double) (0.3F * MathHelper.sin(NarwhalEntity.this.rotationPitch * ((float) Math.PI / 180F))
								* 1.5F),
						(double) (0.3F * MathHelper.cos(NarwhalEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(NarwhalEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.sin(f1) * f2));
				NarwhalEntity.this.world.addEntity(itementity);
			}
		}
	}
	...
	static class SwimToTreasureGoal extends Goal {
		...
		SwimToTreasureGoal(NarwhalEntity dolphinIn) {
			this.dolphin = dolphinIn;
	...
	static class SwimWithPlayerGoal extends Goal {
		...
		SwimWithPlayerGoal(NarwhalEntity dolphinIn, double speedIn) {
			this.dolphin = dolphinIn;
			this.speed = speedIn;
			this.setMutexFlags(EnumSet.of(Goal.Flag.MOVE, Goal.Flag.LOOK));
		}
		 ...
		 */
		public boolean shouldExecute() {
			this.targetPlayer = this.dolphin.world.getClosestPlayer(NarwhalEntity.field_213810_bA, this.dolphin);
