...
public class MegalodonEntity extends AbstractFishEntity {
	private static final DataParameter<Boolean> ATTACKING = EntityDataManager.createKey(MegalodonEntity.class,
			DataSerializers.BOOLEAN);
	public static final int attackTime = 30;
	private static final DataParameter<BlockPos> TREASURE_POS = EntityDataManager.createKey(MegalodonEntity.class,
			DataSerializers.BLOCK_POS);
	private static final DataParameter<Boolean> GOT_FISH = EntityDataManager.createKey(MegalodonEntity.class,
			DataSerializers.BOOLEAN);
	...
	public MegalodonEntity(EntityType<? extends MegalodonEntity> type, World worldIN) {
		super(type, worldIN);
		this.moveController = new MegalodonEntity.MoveHelperController(this);
	...
	protected void registerGoals() {
		...
		//	this.goalSelector.addGoal(4, new HammerheadShark.SwimWithPlayerGoal(this, 4.0D));
	...
	protected void registerAttributes() {
		...
		this.getAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue((double) 0.90F);
		this.getAttributes().registerAttribute(SharedMonsterAttributes.ATTACK_DAMAGE);
		this.getAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(8.0D);
	//	this.getAttribute(SharedMonsterAttributes.FOLLOW_RANGE).setBaseValue(60.0D);
	...
	public boolean attackEntityAsMob(Entity entityIn) {
		...
		if (flag) {
			...
			MegalodonEntity.this.setAttacking(true);
	...
	public static boolean func_223364_b(EntityType<MegalodonEntity> p_223364_0_, IWorld p_223364_1_, SpawnReason reason,
	...
	static class MoveHelperController extends MovementController {
		private final MegalodonEntity dolphin;

		public MoveHelperController(MegalodonEntity dolphinIn) {
			super(dolphinIn);
	...
	class PlayWithItemsGoal extends Goal {
		...
		public boolean shouldExecute() {
			if (this.field_205154_b > MegalodonEntity.this.ticksExisted) {
				return false;
			} else {
				List<ItemEntity> list = MegalodonEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
						MegalodonEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), MegalodonEntity.ITEM_SELECTOR);
				return !list.isEmpty() || !MegalodonEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND).isEmpty();
			}
		}
		...
		public void startExecuting() {
			MegalodonEntity.attackTimer = attackTime;
			List<ItemEntity> list = MegalodonEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
					MegalodonEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), MegalodonEntity.ITEM_SELECTOR);
			if (!list.isEmpty()) {
				MegalodonEntity.this.getNavigator().tryMoveToEntityLiving(list.get(0), (double) 1.2F);
				// ReefSharkEntity.this.playSound(MinenauticaSounds.STALKER_AMBIENT, 1.0F,
				// 1.0F);
			}
		...
		public void resetTask() {
			ItemStack itemstack = MegalodonEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND);
			MegalodonEntity.this.setAttacking(false);
			if (!itemstack.isEmpty()) {
				this.func_220810_a(itemstack);
				MegalodonEntity.this.setItemStackToSlot(EquipmentSlotType.MAINHAND, ItemStack.EMPTY);
				this.field_205154_b = MegalodonEntity.this.ticksExisted + MegalodonEntity.this.rand.nextInt(100);
			}
		...
		private void func_220810_a(ItemStack p_220810_1_) {
			if (!p_220810_1_.isEmpty()) {
				double d0 = MegalodonEntity.this.getPosYEye() - (double) 0.3F;
				ItemEntity itementity = new ItemEntity(MegalodonEntity.this.world, MegalodonEntity.this.getPosX(), d0,
						MegalodonEntity.this.getPosZ(), p_220810_1_);
				itementity.setPickupDelay(40);
				itementity.setThrowerId(MegalodonEntity.this.getUniqueID());
				float f = 0.3F;
				float f1 = MegalodonEntity.this.rand.nextFloat() * ((float) Math.PI * 2F);
				float f2 = 0.02F * MegalodonEntity.this.rand.nextFloat();
				itementity.setMotion(
						(double) (0.3F * -MathHelper.sin(MegalodonEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(MegalodonEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.cos(f1) * f2),
						(double) (0.3F * MathHelper.sin(MegalodonEntity.this.rotationPitch * ((float) Math.PI / 180F))
								* 1.5F),
						(double) (0.3F * MathHelper.cos(MegalodonEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(MegalodonEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.sin(f1) * f2));
				MegalodonEntity.this.world.addEntity(itementity);
			}
		}
	}
	...
	static class SwimToTreasureGoal extends Goal {
		...
		SwimToTreasureGoal(MegalodonEntity dolphinIn) {
			this.dolphin = dolphinIn;
	...
	static class SwimWithPlayerGoal extends Goal {
		...
		SwimWithPlayerGoal(MegalodonEntity dolphinIn, double speedIn) {
			this.dolphin = dolphinIn;
			this.speed = speedIn;
			this.setMutexFlags(EnumSet.of(Goal.Flag.MOVE, Goal.Flag.LOOK));
		}
		 ...
		 */
		public boolean shouldExecute() {
			this.targetPlayer = this.dolphin.world.getClosestPlayer(MegalodonEntity.field_213810_bA, this.dolphin);
