...
import com.viperfish2000.entity.ModBoatEntity.Type;
...
public class ChestBoatEntity extends BoatEntity implements IInventory,INamedContainerProvider  {
   ...
   private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(ChestBoatEntity.class, DataSerializers.VARINT);
   private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(ChestBoatEntity.class, DataSerializers.VARINT);
   private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(ChestBoatEntity.class, DataSerializers.FLOAT);
   private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.createKey(ChestBoatEntity.class, DataSerializers.VARINT);
   private static final DataParameter<Boolean> field_199704_e = EntityDataManager.createKey(ChestBoatEntity.class, DataSerializers.BOOLEAN);
   private static final DataParameter<Boolean> field_199705_f = EntityDataManager.createKey(ChestBoatEntity.class, DataSerializers.BOOLEAN);
   private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(ChestBoatEntity.class, DataSerializers.VARINT);
   ...
   private ChestBoatEntity.Status status;
   private ChestBoatEntity.Status previousStatus;
   ...
   public ChestBoatEntity(EntityType<? extends ChestBoatEntity> p_i50129_1_, World p_i50129_2_) {
      super(p_i50129_1_, p_i50129_2_);
      this.preventEntitySpawning = true;
   }

   public ChestBoatEntity(World worldIn, double x, double y, double z) {
      this(CrocodileEntities.CHEST_BOAT, worldIn);
   ...
   public ChestBoatEntity(FMLPlayMessages.SpawnEntity spawnEntity, World world) {
	    this(CrocodileEntities.CHEST_BOAT, world);
	  }
   ...
   protected void registerData() {
      ...
      this.dataManager.register(BOAT_TYPE, ChestBoatEntity.Type.OAK.ordinal());
   ...
   public void applyEntityCollision(Entity entityIn) {
      if (entityIn instanceof ChestBoatEntity) {
   ...
   public void tick() {
      ...
      if (this.status != ChestBoatEntity.Status.UNDER_WATER && this.status != ChestBoatEntity.Status.UNDER_FLOWING_WATER) {
    ...
    */
   private ChestBoatEntity.Status getBoatStatus() {
      ChestBoatEntity.Status boatentity$status = this.getUnderwaterStatus();
      if (boatentity$status != null) {
         this.waterLevel = this.getBoundingBox().maxY;
         return boatentity$status;
      } else if (this.checkInWater()) {
         return ChestBoatEntity.Status.IN_WATER;
      } else {
         float f = this.getBoatGlide();
         if (f > 0.0F) {
            this.boatGlide = f;
            return ChestBoatEntity.Status.ON_LAND;
         } else {
            return ChestBoatEntity.Status.IN_AIR;
         }
      }
   }
    ...
    */
   @Nullable
   private ChestBoatEntity.Status getUnderwaterStatus() {
      AxisAlignedBB axisalignedbb = this.getBoundingBox();
               ...
               for(int i2 = i1; i2 < j1; ++i2) {
                  ...
                  if (ifluidstate.isTagged(FluidTags.WATER) && d0 < (double)((float)blockpos$pooledmutable.getY() + ifluidstate.getActualHeight(this.world, blockpos$pooledmutable))) {
                     if (!ifluidstate.isSource()) {
                        ChestBoatEntity.Status boatentity$status = ChestBoatEntity.Status.UNDER_FLOWING_WATER;
                        return boatentity$status;
                     }
      ...
      return flag ? ChestBoatEntity.Status.UNDER_WATER : null;
   }
   ...
   private void updateMotion() {
      ...
      double d2 = 0.0D;
      this.momentum = 0.05F;
      if (this.previousStatus == ChestBoatEntity.Status.IN_AIR && this.status != ChestBoatEntity.Status.IN_AIR && this.status != ChestBoatEntity.Status.ON_LAND) {
         ...
         this.lastYd = 0.0D;
         this.status = ChestBoatEntity.Status.IN_WATER;
      } else {
         if (this.status == ChestBoatEntity.Status.IN_WATER) {
            d2 = (this.waterLevel - this.getPosY()) / (double)this.getHeight();
            this.momentum = 0.9F;
         } else if (this.status == ChestBoatEntity.Status.UNDER_FLOWING_WATER) {
            d1 = -7.0E-4D;
            this.momentum = 0.9F;
         } else if (this.status == ChestBoatEntity.Status.UNDER_WATER) {
            d2 = (double)0.01F;
            this.momentum = 0.45F;
         } else if (this.status == ChestBoatEntity.Status.IN_AIR) {
            this.momentum = 0.9F;
         } else if (this.status == ChestBoatEntity.Status.ON_LAND) {
      ...
      if (!this.isPassenger()) {
         if (onGroundIn) {
            if (this.fallDistance > 3.0F) {
               if (this.status != ChestBoatEntity.Status.ON_LAND) {
   ...
   public void setBoatType(ChestBoatEntity.Type boatType) {
