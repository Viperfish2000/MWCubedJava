...
public class ThresherEntity extends AbstractFishEntity {
	private static final DataParameter<Boolean> ATTACKING = EntityDataManager.createKey(ThresherEntity.class,
			DataSerializers.BOOLEAN);
	public static final int attackTime = 30;
	private static final DataParameter<BlockPos> TREASURE_POS = EntityDataManager.createKey(ThresherEntity.class,
			DataSerializers.BLOCK_POS);
	private static final DataParameter<Boolean> GOT_FISH = EntityDataManager.createKey(ThresherEntity.class,
			DataSerializers.BOOLEAN);
	...
	public ThresherEntity(EntityType<? extends ThresherEntity> type, World worldIN) {
		super(type, worldIN);
		this.moveController = new ThresherEntity.MoveHelperController(this);
	...
	protected void registerGoals() {
		...
		//this.goalSelector.addGoal(4, new ThresherEntity.SwimWithPlayerGoal(this, 4.0D));
	...
	//	this.goalSelector.addGoal(9, new AvoidEntityGoal<>(this, PlayerEntity.class, 8.0F, 1.0D, 1.0D));
		...
		this.goalSelector.addGoal(5, new EntityModJump(this, 10));
	...
	protected void registerAttributes() {
		...
		this.getAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue((double) 1.25F);
	...
	public boolean attackEntityAsMob(Entity entityIn) {
		...
		//	this.playSound(MinenauticaSounds.STALKER_ATTACK, 1.0F, 1.0F);
			...
			ThresherEntity.this.setAttacking(true);
	...
	public static boolean func_223364_b(EntityType<ThresherEntity> p_223364_0_, IWorld p_223364_1_, SpawnReason reason,
	...
	static class MoveHelperController extends MovementController {
		private final ThresherEntity dolphin;

		public MoveHelperController(ThresherEntity dolphinIn) {
			super(dolphinIn);
	...
	class PlayWithItemsGoal extends Goal {
		...
		public boolean shouldExecute() {
			if (this.field_205154_b > ThresherEntity.this.ticksExisted) {
				return false;
			} else {
				List<ItemEntity> list = ThresherEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
						ThresherEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), ThresherEntity.ITEM_SELECTOR);
				return !list.isEmpty()
						|| !ThresherEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND).isEmpty();
			}
		}
		...
		public void startExecuting() {
			ThresherEntity.attackTimer = attackTime;
			List<ItemEntity> list = ThresherEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
					ThresherEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), ThresherEntity.ITEM_SELECTOR);
			if (!list.isEmpty()) {
				ThresherEntity.this.getNavigator().tryMoveToEntityLiving(list.get(0), (double) 1.2F);
		...
		public void resetTask() {
			ItemStack itemstack = ThresherEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND);
			ThresherEntity.this.setAttacking(false);
			if (!itemstack.isEmpty()) {
				this.func_220810_a(itemstack);
				ThresherEntity.this.setItemStackToSlot(EquipmentSlotType.MAINHAND, ItemStack.EMPTY);
				this.field_205154_b = ThresherEntity.this.ticksExisted + ThresherEntity.this.rand.nextInt(100);
			}
		...
		private void func_220810_a(ItemStack p_220810_1_) {
			if (!p_220810_1_.isEmpty()) {
				double d0 = ThresherEntity.this.getPosYEye() - (double) 0.3F;
				ItemEntity itementity = new ItemEntity(ThresherEntity.this.world, ThresherEntity.this.getPosX(), d0,
						ThresherEntity.this.getPosZ(), p_220810_1_);
				itementity.setPickupDelay(40);
				itementity.setThrowerId(ThresherEntity.this.getUniqueID());
				float f = 0.3F;
				float f1 = ThresherEntity.this.rand.nextFloat() * ((float) Math.PI * 2F);
				float f2 = 0.02F * ThresherEntity.this.rand.nextFloat();
				itementity.setMotion(
						(double) (0.3F * -MathHelper.sin(ThresherEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(ThresherEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.cos(f1) * f2),
						(double) (0.3F * MathHelper.sin(ThresherEntity.this.rotationPitch * ((float) Math.PI / 180F))
								* 1.5F),
						(double) (0.3F * MathHelper.cos(ThresherEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(ThresherEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.sin(f1) * f2));
				ThresherEntity.this.world.addEntity(itementity);
			}
		}
	}
	...
	static class SwimToTreasureGoal extends Goal {
		...
		SwimToTreasureGoal(ThresherEntity dolphinIn) {
			this.dolphin = dolphinIn;
	...
	static class SwimWithPlayerGoal extends Goal {
		...
		SwimWithPlayerGoal(ThresherEntity dolphinIn, double speedIn) {
			this.dolphin = dolphinIn;
			this.speed = speedIn;
			this.setMutexFlags(EnumSet.of(Goal.Flag.MOVE, Goal.Flag.LOOK));
		}
		 ...
		 */
		public boolean shouldExecute() {
			this.targetPlayer = this.dolphin.world.getClosestPlayer(ThresherEntity.field_213810_bA, this.dolphin);
