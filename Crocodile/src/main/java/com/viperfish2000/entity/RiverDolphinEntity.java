...
public class RiverDolphinEntity extends AbstractGroupFishEntity {

	private static final DataParameter<BlockPos> TREASURE_POS = EntityDataManager.createKey(RiverDolphinEntity.class,
			DataSerializers.BLOCK_POS);
	private static final DataParameter<Boolean> GOT_FISH = EntityDataManager.createKey(RiverDolphinEntity.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> MOISTNESS = EntityDataManager.createKey(RiverDolphinEntity.class,
	...
	public RiverDolphinEntity(EntityType<? extends RiverDolphinEntity> type, World worldIN) {
		super(type, worldIN);
		this.moveController = new RiverDolphinEntity.MoveHelperController(this);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new RiverDolphinEntity.SwimToTreasureGoal(this));
		this.goalSelector.addGoal(2, new RiverDolphinEntity.SwimWithPlayerGoal(this, 4.0D));
		this.goalSelector.addGoal(4, new RandomSwimmingGoal(this, 1.0D, 10));
		this.goalSelector.addGoal(4, new LookRandomlyGoal(this));
		this.goalSelector.addGoal(5, new LookAtGoal(this, PlayerEntity.class, 6.0F));
		this.goalSelector.addGoal(5, new RiverDolphinJump(this, 10));
		this.goalSelector.addGoal(6, new MeleeAttackGoal(this, (double) 1.2F, true));
		this.goalSelector.addGoal(8, new RiverDolphinEntity.PlayWithItemsGoal());
	...
	public static boolean func_223364_b(EntityType<RiverDolphinEntity> p_223364_0_, IWorld p_223364_1_, SpawnReason reason,
	...
	static class MoveHelperController extends MovementController {
		private final RiverDolphinEntity dolphin;

		public MoveHelperController(RiverDolphinEntity dolphinIn) {
			super(dolphinIn);
	...
	class PlayWithItemsGoal extends Goal {
		...
		public boolean shouldExecute() {
			if (this.field_205154_b > RiverDolphinEntity.this.ticksExisted) {
				return false;
			} else {
				List<ItemEntity> list = RiverDolphinEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
						RiverDolphinEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), RiverDolphinEntity.ITEM_SELECTOR);
				return !list.isEmpty()
						|| !RiverDolphinEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND).isEmpty();
			}
		}
		...
		public void startExecuting() {
			List<ItemEntity> list = RiverDolphinEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
					RiverDolphinEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), RiverDolphinEntity.ITEM_SELECTOR);
			if (!list.isEmpty()) {
				RiverDolphinEntity.this.getNavigator().tryMoveToEntityLiving(list.get(0), (double) 1.2F);
				RiverDolphinEntity.this.playSound(SoundEvents.ENTITY_DOLPHIN_PLAY, 1.0F, 1.0F);
			}
		...
		public void resetTask() {
			ItemStack itemstack = RiverDolphinEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND);
			if (!itemstack.isEmpty()) {
				this.func_220810_a(itemstack);
				RiverDolphinEntity.this.setItemStackToSlot(EquipmentSlotType.MAINHAND, ItemStack.EMPTY);
				this.field_205154_b = RiverDolphinEntity.this.ticksExisted + RiverDolphinEntity.this.rand.nextInt(100);
			}
		...
		private void func_220810_a(ItemStack p_220810_1_) {
			if (!p_220810_1_.isEmpty()) {
				double d0 = RiverDolphinEntity.this.getPosYEye() - (double) 0.3F;
				ItemEntity itementity = new ItemEntity(RiverDolphinEntity.this.world, RiverDolphinEntity.this.getPosX(), d0,
						RiverDolphinEntity.this.getPosZ(), p_220810_1_);
				itementity.setPickupDelay(40);
				itementity.setThrowerId(RiverDolphinEntity.this.getUniqueID());
				float f = 0.3F;
				float f1 = RiverDolphinEntity.this.rand.nextFloat() * ((float) Math.PI * 2F);
				float f2 = 0.02F * RiverDolphinEntity.this.rand.nextFloat();
				itementity.setMotion(
						(double) (0.3F * -MathHelper.sin(RiverDolphinEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(RiverDolphinEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.cos(f1) * f2),
						(double) (0.3F * MathHelper.sin(RiverDolphinEntity.this.rotationPitch * ((float) Math.PI / 180F))
								* 1.5F),
						(double) (0.3F * MathHelper.cos(RiverDolphinEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(RiverDolphinEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.sin(f1) * f2));
				RiverDolphinEntity.this.world.addEntity(itementity);
			}
		}
	}
	...
	static class SwimToTreasureGoal extends Goal {
		...
		SwimToTreasureGoal(RiverDolphinEntity dolphinIn) {
			this.dolphin = dolphinIn;
	...
	static class SwimWithPlayerGoal extends Goal {
		...
		SwimWithPlayerGoal(RiverDolphinEntity dolphinIn, double speedIn) {
			this.dolphin = dolphinIn;
			this.speed = speedIn;
			this.setMutexFlags(EnumSet.of(Goal.Flag.MOVE, Goal.Flag.LOOK));
		}
		 ...
		 */
		public boolean shouldExecute() {
			this.targetPlayer = this.dolphin.world.getClosestPlayer(RiverDolphinEntity.field_213810_bA, this.dolphin);
