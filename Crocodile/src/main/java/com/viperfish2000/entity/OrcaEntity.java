...
public class OrcaEntity extends AnimalEntity {

	private static final DataParameter<BlockPos> TREASURE_POS = EntityDataManager.createKey(OrcaEntity.class,
			DataSerializers.BLOCK_POS);
	private static final DataParameter<Boolean> GOT_FISH = EntityDataManager.createKey(OrcaEntity.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> MOISTNESS = EntityDataManager.createKey(OrcaEntity.class,
	...
	public OrcaEntity(EntityType<? extends OrcaEntity> type, World worldIN) {
		super(type, worldIN);
		this.moveController = new OrcaEntity.MoveHelperController(this);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new OrcaEntity.SwimToTreasureGoal(this));
		this.goalSelector.addGoal(2, new OrcaEntity.SwimWithPlayerGoal(this, 4.0D));
	...
	//	this.goalSelector.addGoal(5, new DolphinJumpGoal(this, 10));
		...
		this.goalSelector.addGoal(8, new OrcaEntity.PlayWithItemsGoal());
	...
	public static boolean func_223364_b(EntityType<OrcaEntity> p_223364_0_, IWorld p_223364_1_, SpawnReason reason,
		...
		public void tick() {
			...
			if (this.action == MovementController.Action.MOVE_TO && !this.dolphin.getNavigator().noPath()) {
				...
				if (d3 < (double) 2.5000003E-7F) {
					...
					this.dolphin.rotationYaw = this.limitAngle(this.dolphin.rotationYaw, f, 6.0F);
					...
					if (this.dolphin.isInWater()) {
						...
						this.dolphin.rotationPitch = this.limitAngle(this.dolphin.rotationPitch, f2, 3.0F);
	...
	class PlayWithItemsGoal extends Goal {
		...
		public boolean shouldExecute() {
			if (this.field_205154_b > OrcaEntity.this.ticksExisted) {
				return false;
			} else {
				List<ItemEntity> list = OrcaEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
						OrcaEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), OrcaEntity.ITEM_SELECTOR);
				return !list.isEmpty()
						|| !OrcaEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND).isEmpty();
			}
		}
		...
		public void startExecuting() {
			List<ItemEntity> list = OrcaEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
					OrcaEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), OrcaEntity.ITEM_SELECTOR);
			if (!list.isEmpty()) {
				OrcaEntity.this.getNavigator().tryMoveToEntityLiving(list.get(0), (double) 1.2F);
				OrcaEntity.this.playSound(SoundEvents.ENTITY_DOLPHIN_PLAY, 1.0F, 1.0F);
			}
		...
		public void resetTask() {
			ItemStack itemstack = OrcaEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND);
			if (!itemstack.isEmpty()) {
				this.func_220810_a(itemstack);
				OrcaEntity.this.setItemStackToSlot(EquipmentSlotType.MAINHAND, ItemStack.EMPTY);
				this.field_205154_b = OrcaEntity.this.ticksExisted + OrcaEntity.this.rand.nextInt(100);
			}
		...
		private void func_220810_a(ItemStack p_220810_1_) {
			if (!p_220810_1_.isEmpty()) {
				double d0 = OrcaEntity.this.getPosYEye() - (double) 0.3F;
				ItemEntity itementity = new ItemEntity(OrcaEntity.this.world, OrcaEntity.this.getPosX(), d0,
						OrcaEntity.this.getPosZ(), p_220810_1_);
				itementity.setPickupDelay(40);
				itementity.setThrowerId(OrcaEntity.this.getUniqueID());
				float f = 0.3F;
				float f1 = OrcaEntity.this.rand.nextFloat() * ((float) Math.PI * 2F);
				float f2 = 0.02F * OrcaEntity.this.rand.nextFloat();
				itementity.setMotion(
						(double) (0.3F * -MathHelper.sin(OrcaEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(OrcaEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.cos(f1) * f2),
						(double) (0.3F * MathHelper.sin(OrcaEntity.this.rotationPitch * ((float) Math.PI / 180F))
								* 1.5F),
						(double) (0.3F * MathHelper.cos(OrcaEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(OrcaEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.sin(f1) * f2));
				OrcaEntity.this.world.addEntity(itementity);
			}
		}
	}
	...
	static class SwimToTreasureGoal extends Goal {
		...
		SwimToTreasureGoal(OrcaEntity dolphinIn) {
			this.dolphin = dolphinIn;
	...
	static class SwimWithPlayerGoal extends Goal {
		...
		SwimWithPlayerGoal(OrcaEntity dolphinIn, double speedIn) {
			this.dolphin = dolphinIn;
			this.speed = speedIn;
			this.setMutexFlags(EnumSet.of(Goal.Flag.MOVE, Goal.Flag.LOOK));
		}
		 ...
		 */
		public boolean shouldExecute() {
			this.targetPlayer = this.dolphin.world.getClosestPlayer(OrcaEntity.field_213810_bA, this.dolphin);
	...
	public AgeableEntity createChild(AgeableEntity ageable) {
		// TODO Auto-generated method stub
		  return CrocodileEntities.ORCA.create(this.world);
	}
}