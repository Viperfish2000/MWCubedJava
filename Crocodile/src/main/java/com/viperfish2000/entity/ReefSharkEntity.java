...
public class ReefSharkEntity extends AbstractFishEntity {
	private static final DataParameter<Boolean> ATTACKING = EntityDataManager.createKey(ReefSharkEntity.class,
			DataSerializers.BOOLEAN);
	public static final int attackTime = 30;
	private static final DataParameter<BlockPos> TREASURE_POS = EntityDataManager.createKey(ReefSharkEntity.class,
			DataSerializers.BLOCK_POS);
	private static final DataParameter<Boolean> GOT_FISH = EntityDataManager.createKey(ReefSharkEntity.class,
			DataSerializers.BOOLEAN);
	...
	public ReefSharkEntity(EntityType<? extends ReefSharkEntity> type, World worldIN) {
		super(type, worldIN);
		this.moveController = new ReefSharkEntity.MoveHelperController(this);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(4, new ReefSharkEntity.SwimWithPlayerGoal(this, 4.0D));
	...
	//	this.goalSelector.addGoal(1, new ReefSharkEntity.PlayWithItemsGoal());
		...
		this.goalSelector.addGoal(9, new AvoidEntityGoal<>(this, PlayerEntity.class, 5.0F, 1.0D, 1.0D));
	...
	public boolean attackEntityAsMob(Entity entityIn) {
		...
		//	this.playSound(MinenauticaSounds.STALKER_ATTACK, 1.0F, 1.0F);
			...
			ReefSharkEntity.this.setAttacking(true);
	...
	public static boolean func_223364_b(EntityType<ReefSharkEntity> p_223364_0_, IWorld p_223364_1_, SpawnReason reason,
	...
	static class MoveHelperController extends MovementController {
		private final ReefSharkEntity dolphin;

		public MoveHelperController(ReefSharkEntity dolphinIn) {
			super(dolphinIn);
	...
	class PlayWithItemsGoal extends Goal {
		...
		public boolean shouldExecute() {
			if (this.field_205154_b > ReefSharkEntity.this.ticksExisted) {
				return false;
			} else {
				List<ItemEntity> list = ReefSharkEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
						ReefSharkEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), ReefSharkEntity.ITEM_SELECTOR);
				return !list.isEmpty()
						|| !ReefSharkEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND).isEmpty();
			}
		}
		...
		public void startExecuting() {
			ReefSharkEntity.attackTimer = attackTime;
			List<ItemEntity> list = ReefSharkEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
					ReefSharkEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), ReefSharkEntity.ITEM_SELECTOR);
			if (!list.isEmpty()) {
				ReefSharkEntity.this.getNavigator().tryMoveToEntityLiving(list.get(0), (double) 1.2F);
		...
		public void resetTask() {
			ItemStack itemstack = ReefSharkEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND);
			ReefSharkEntity.this.setAttacking(false);
			if (!itemstack.isEmpty()) {
				this.func_220810_a(itemstack);
				ReefSharkEntity.this.setItemStackToSlot(EquipmentSlotType.MAINHAND, ItemStack.EMPTY);
				this.field_205154_b = ReefSharkEntity.this.ticksExisted + ReefSharkEntity.this.rand.nextInt(100);
			}
		...
		private void func_220810_a(ItemStack p_220810_1_) {
			if (!p_220810_1_.isEmpty()) {
				double d0 = ReefSharkEntity.this.getPosYEye() - (double) 0.3F;
				ItemEntity itementity = new ItemEntity(ReefSharkEntity.this.world, ReefSharkEntity.this.getPosX(), d0,
						ReefSharkEntity.this.getPosZ(), p_220810_1_);
				itementity.setPickupDelay(40);
				itementity.setThrowerId(ReefSharkEntity.this.getUniqueID());
				float f = 0.3F;
				float f1 = ReefSharkEntity.this.rand.nextFloat() * ((float) Math.PI * 2F);
				float f2 = 0.02F * ReefSharkEntity.this.rand.nextFloat();
				itementity.setMotion(
						(double) (0.3F * -MathHelper.sin(ReefSharkEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(ReefSharkEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.cos(f1) * f2),
						(double) (0.3F * MathHelper.sin(ReefSharkEntity.this.rotationPitch * ((float) Math.PI / 180F))
								* 1.5F),
						(double) (0.3F * MathHelper.cos(ReefSharkEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(ReefSharkEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.sin(f1) * f2));
				ReefSharkEntity.this.world.addEntity(itementity);
			}
		}
	}
	...
	static class SwimToTreasureGoal extends Goal {
		...
		SwimToTreasureGoal(ReefSharkEntity dolphinIn) {
			this.dolphin = dolphinIn;
	...
	static class SwimWithPlayerGoal extends Goal {
		...
		SwimWithPlayerGoal(ReefSharkEntity dolphinIn, double speedIn) {
			this.dolphin = dolphinIn;
			this.speed = speedIn;
			this.setMutexFlags(EnumSet.of(Goal.Flag.MOVE, Goal.Flag.LOOK));
		}
		 ...
		 */
		public boolean shouldExecute() {
			this.targetPlayer = this.dolphin.world.getClosestPlayer(ReefSharkEntity.field_213810_bA, this.dolphin);
