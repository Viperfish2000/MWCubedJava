...
public class RiverDolphinJump extends JumpGoal {
	private static final int[] JUMP_DISTANCES = new int[] { 0, 1, 4, 5, 6, 7 };
	private final RiverDolphinEntity field_220711_b;
	private final int field_220712_c;
	private boolean field_220713_d;

	public RiverDolphinJump(RiverDolphinEntity p_i50329_1_, int p_i50329_2_) {
	...
	private boolean canJumpTo(BlockPos pos, int dx, int dz, int scale) {
		...
		return this.field_220711_b.world.getFluidState(blockpos).isTagged(FluidTags.WATER)
				&& !this.field_220711_b.world.getBlockState(blockpos).getMaterial().blocksMovement();
	}

	private boolean isAirAbove(BlockPos pos, int dx, int dz, int scale) {
		return this.field_220711_b.world.getBlockState(pos.add(dx * scale, 1, dz * scale)).isAir()
				&& this.field_220711_b.world.getBlockState(pos.add(dx * scale, 2, dz * scale)).isAir();
	}
	...
	public boolean shouldContinueExecuting() {
		...
		return (!(d0 * d0 < (double) 0.03F) || this.field_220711_b.rotationPitch == 0.0F
				|| !(Math.abs(this.field_220711_b.rotationPitch) < 10.0F) || !this.field_220711_b.isInWater())
				&& !this.field_220711_b.onGround;
	}
	...
	public void startExecuting() {
		Direction direction = this.field_220711_b.getAdjustedHorizontalFacing();
		this.field_220711_b.setMotion(this.field_220711_b.getMotion().add((double) direction.getXOffset() * 0.6D, 0.7D,
