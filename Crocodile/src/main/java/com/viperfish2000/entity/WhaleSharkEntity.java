...
public class WhaleSharkEntity extends AbstractFishEntity {
	private static final DataParameter<Boolean> ATTACKING = EntityDataManager.createKey(WhaleSharkEntity.class,
			DataSerializers.BOOLEAN);
	public static final int attackTime = 30;
	private static final DataParameter<BlockPos> TREASURE_POS = EntityDataManager.createKey(WhaleSharkEntity.class,
			DataSerializers.BLOCK_POS);
	private static final DataParameter<Boolean> GOT_FISH = EntityDataManager.createKey(WhaleSharkEntity.class,
			DataSerializers.BOOLEAN);
	...
	public WhaleSharkEntity(EntityType<? extends WhaleSharkEntity> type, World worldIN) {
		super(type, worldIN);
		this.moveController = new WhaleSharkEntity.MoveHelperController(this);
	...
	//	this.goalSelector.addGoal(4, new HammerheadShark.SwimWithPlayerGoal(this, 4.0D));
		this.goalSelector.addGoal(3, new RandomSwimmingGoal(this, 1.0D, 8));
	...
	//	this.goalSelector.addGoal(9, new AvoidEntityGoal<>(this, PlayerEntity.class, 2.0F, 1.0D, 1.0D));
	...
	public boolean attackEntityAsMob(Entity entityIn) {
		...
		//	this.playSound(MinenauticaSounds.STALKER_ATTACK, 1.0F, 1.0F);
			...
			WhaleSharkEntity.this.setAttacking(true);
	...
	public static boolean func_223364_b(EntityType<WhaleSharkEntity> p_223364_0_, IWorld p_223364_1_, SpawnReason reason,
		...
		public void tick() {
			...
			if (this.action == MovementController.Action.MOVE_TO && !this.dolphin.getNavigator().noPath()) {
				...
				if (d3 < (double) 2.5000003E-7F) {
					...
					this.dolphin.rotationYaw = this.limitAngle(this.dolphin.rotationYaw, f, 2.0F);
					...
					if (this.dolphin.isInWater()) {
						...
						this.dolphin.rotationPitch = this.limitAngle(this.dolphin.rotationPitch, f2,2.0F);
	...
	class PlayWithItemsGoal extends Goal {
		...
		public boolean shouldExecute() {
			if (this.field_205154_b > WhaleSharkEntity.this.ticksExisted) {
				return false;
			} else {
				List<ItemEntity> list = WhaleSharkEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
						WhaleSharkEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), WhaleSharkEntity.ITEM_SELECTOR);
				return !list.isEmpty()
						|| !WhaleSharkEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND).isEmpty();
			}
		}
		...
		public void startExecuting() {
			WhaleSharkEntity.attackTimer = attackTime;
			List<ItemEntity> list = WhaleSharkEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
					WhaleSharkEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), WhaleSharkEntity.ITEM_SELECTOR);
			if (!list.isEmpty()) {
				WhaleSharkEntity.this.getNavigator().tryMoveToEntityLiving(list.get(0), (double) 1.2F);
		...
		public void resetTask() {
			ItemStack itemstack = WhaleSharkEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND);
			WhaleSharkEntity.this.setAttacking(false);
			if (!itemstack.isEmpty()) {
				this.func_220810_a(itemstack);
				WhaleSharkEntity.this.setItemStackToSlot(EquipmentSlotType.MAINHAND, ItemStack.EMPTY);
				this.field_205154_b = WhaleSharkEntity.this.ticksExisted + WhaleSharkEntity.this.rand.nextInt(100);
			}
		...
		private void func_220810_a(ItemStack p_220810_1_) {
			if (!p_220810_1_.isEmpty()) {
				double d0 = WhaleSharkEntity.this.getPosYEye() - (double) 0.3F;
				ItemEntity itementity = new ItemEntity(WhaleSharkEntity.this.world, WhaleSharkEntity.this.getPosX(), d0,
						WhaleSharkEntity.this.getPosZ(), p_220810_1_);
				itementity.setPickupDelay(40);
				itementity.setThrowerId(WhaleSharkEntity.this.getUniqueID());
				float f = 0.3F;
				float f1 = WhaleSharkEntity.this.rand.nextFloat() * ((float) Math.PI * 2F);
				float f2 = 0.02F * WhaleSharkEntity.this.rand.nextFloat();
				itementity.setMotion(
						(double) (0.3F * -MathHelper.sin(WhaleSharkEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(WhaleSharkEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.cos(f1) * f2),
						(double) (0.3F * MathHelper.sin(WhaleSharkEntity.this.rotationPitch * ((float) Math.PI / 180F))
								* 1.5F),
						(double) (0.3F * MathHelper.cos(WhaleSharkEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(WhaleSharkEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.sin(f1) * f2));
				WhaleSharkEntity.this.world.addEntity(itementity);
			}
		}
	}
	...
	static class SwimToTreasureGoal extends Goal {
		...
		SwimToTreasureGoal(WhaleSharkEntity dolphinIn) {
			this.dolphin = dolphinIn;
	...
	static class SwimWithPlayerGoal extends Goal {
		...
		SwimWithPlayerGoal(WhaleSharkEntity dolphinIn, double speedIn) {
			this.dolphin = dolphinIn;
			this.speed = speedIn;
			this.setMutexFlags(EnumSet.of(Goal.Flag.MOVE, Goal.Flag.LOOK));
		}
		 ...
		 */
		public boolean shouldExecute() {
			this.targetPlayer = this.dolphin.world.getClosestPlayer(WhaleSharkEntity.field_213810_bA, this.dolphin);
