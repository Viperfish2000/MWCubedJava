...
public class CrewmateEntity extends TameableEntity {
	private static final DataParameter<Boolean> BEGGING = EntityDataManager.createKey(CrewmateEntity.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> COLLAR_COLOR = EntityDataManager.createKey(CrewmateEntity.class,
			DataSerializers.VARINT);
	   private static final DataParameter<Integer> CREWMATE_TYPE = EntityDataManager.createKey(CrewmateEntity.class, DataSerializers.VARINT);
	...
	public CrewmateEntity(EntityType<? extends CrewmateEntity> type, World worldIn) {
		super(type, worldIn);
		this.setTamed(false);
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(3, new CrewmateEntity.AvoidEntityGoal(this, LlamaEntity.class, 24.0F, 1.5D, 1.5D));
	...
	protected void registerData() {
		...
		this.dataManager.register(COLLAR_COLOR, DyeColor.RED.getId());
	     this.dataManager.register(CREWMATE_TYPE, 0);
	}
	...
	public void writeAdditional(CompoundNBT compound) {
		...
		compound.putInt("CrewmateType", this.getCrewmateType());
	...
	public void readAdditional(CompoundNBT compound) {
		...
		this.setCrewmateType(compound.getInt("CrewmateType"));
	...
	public int getCrewmateType() {
		return this.dataManager.get(CREWMATE_TYPE);
	}

	public int getVariant() {
		return MathHelper.clamp(this.dataManager.get(CREWMATE_TYPE), 0, 3);
	}

	public void setCrewmateType(int butterflyTypeId) {

		this.dataManager.set(CREWMATE_TYPE, butterflyTypeId);
	}

	private int getRandomCrewmateType(IWorld p_213610_1_) {
	...
	public ILivingEntityData onInitialSpawn(IWorld worldIn, DifficultyInstance difficultyIn, SpawnReason reason,
		...
		if (dataTag != null && dataTag.contains("BucketVariantTag", 3)) {
			this.setCrewmateType(dataTag.getInt("BucketVariantTag"));
			return spawnDataIn;
		} else {
			spawnDataIn = super.onInitialSpawn(worldIn, difficultyIn, reason, spawnDataIn, dataTag);
			int i = this.getRandomCrewmateType(worldIn);
	...
	public CrewmateEntity createChild(AgeableEntity ageable) {
		CrewmateEntity CrewmateEntity = CrocodileEntities.CREWMATE.create(this.world);
		UUID uuid = this.getOwnerId();
		if (uuid != null) {
			CrewmateEntity.setOwnerId(uuid);
			CrewmateEntity.setTamed(true);
		}

		return CrewmateEntity;
	}
	...
	public boolean canMateWith(AnimalEntity otherAnimal) {
		if (otherAnimal == this) {
			...
			return false;
		} else if (!(otherAnimal instanceof CrewmateEntity)) {
			return false;
		} else {
			CrewmateEntity CrewmateEntity = (CrewmateEntity) otherAnimal;
			if (!CrewmateEntity.isTamed()) {
				return false;
			} else if (CrewmateEntity.isSitting()) {
				return false;
			} else {
				return this.isInLove() && CrewmateEntity.isInLove();
			}
		}
	}
	...
	public boolean shouldAttackEntity(LivingEntity target, LivingEntity owner) {
		if (!(target instanceof CreeperEntity) && !(target instanceof GhastEntity)) {
			if (target instanceof CrewmateEntity) {
				CrewmateEntity CrewmateEntity = (CrewmateEntity) target;
				return !CrewmateEntity.isTamed() || CrewmateEntity.getOwner() != owner;
	...
	class AvoidEntityGoal<T extends LivingEntity> extends net.minecraft.entity.ai.goal.AvoidEntityGoal<T> {
		private final CrewmateEntity wolf;

		public AvoidEntityGoal(CrewmateEntity wolfIn, Class<T> entityClassToAvoidIn, float avoidDistanceIn,
		...
		private boolean avoidLlama(LlamaEntity llamaIn) {
			return llamaIn.getStrength() >= CrewmateEntity.this.rand.nextInt(5);
		}
		...
		public void startExecuting() {
			CrewmateEntity.this.setAttackTarget((LivingEntity) null);
