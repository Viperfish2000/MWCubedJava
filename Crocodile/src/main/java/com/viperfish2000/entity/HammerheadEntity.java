...
public class HammerheadEntity extends AbstractFishEntity {
	private static final DataParameter<Boolean> ATTACKING = EntityDataManager.createKey(HammerheadEntity.class,
			DataSerializers.BOOLEAN);
	public static final int attackTime = 30;
	private static final DataParameter<BlockPos> TREASURE_POS = EntityDataManager.createKey(HammerheadEntity.class,
			DataSerializers.BLOCK_POS);
	private static final DataParameter<Boolean> GOT_FISH = EntityDataManager.createKey(HammerheadEntity.class,
			DataSerializers.BOOLEAN);
	...
	protected void registerAttributes() {
		...
		this.getAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue((double) 0.75F);
	...
	public boolean attackEntityAsMob(Entity entityIn) {
		...
		//	this.playSound(MinenauticaSounds.STALKER_ATTACK, 1.0F, 1.0F);
			...
			HammerheadEntity.this.setAttacking(true);
	...
	public static boolean func_223364_b(EntityType<HammerheadEntity> p_223364_0_, IWorld p_223364_1_, SpawnReason reason,
	...
	static class MoveHelperController extends MovementController {
		private final HammerheadEntity dolphin;

		public MoveHelperController(HammerheadEntity dolphinIn) {
			super(dolphinIn);
	...
	class PlayWithItemsGoal extends Goal {
		...
		public boolean shouldExecute() {
			if (this.field_205154_b > HammerheadEntity.this.ticksExisted) {
				return false;
			} else {
				List<ItemEntity> list = HammerheadEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
						HammerheadEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), HammerheadEntity.ITEM_SELECTOR);
				return !list.isEmpty()
						|| !HammerheadEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND).isEmpty();
			}
		}
		...
		public void startExecuting() {
			HammerheadEntity.attackTimer = attackTime;
			List<ItemEntity> list = HammerheadEntity.this.world.getEntitiesWithinAABB(ItemEntity.class,
					HammerheadEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), HammerheadEntity.ITEM_SELECTOR);
			if (!list.isEmpty()) {
				HammerheadEntity.this.getNavigator().tryMoveToEntityLiving(list.get(0), (double) 1.2F);
		...
		public void resetTask() {
			ItemStack itemstack = HammerheadEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND);
			HammerheadEntity.this.setAttacking(false);
			if (!itemstack.isEmpty()) {
				this.func_220810_a(itemstack);
				HammerheadEntity.this.setItemStackToSlot(EquipmentSlotType.MAINHAND, ItemStack.EMPTY);
				this.field_205154_b = HammerheadEntity.this.ticksExisted + HammerheadEntity.this.rand.nextInt(100);
			}
		...
		private void func_220810_a(ItemStack p_220810_1_) {
			if (!p_220810_1_.isEmpty()) {
				double d0 = HammerheadEntity.this.getPosYEye() - (double) 0.3F;
				ItemEntity itementity = new ItemEntity(HammerheadEntity.this.world, HammerheadEntity.this.getPosX(), d0,
						HammerheadEntity.this.getPosZ(), p_220810_1_);
				itementity.setPickupDelay(40);
				itementity.setThrowerId(HammerheadEntity.this.getUniqueID());
				float f = 0.3F;
				float f1 = HammerheadEntity.this.rand.nextFloat() * ((float) Math.PI * 2F);
				float f2 = 0.02F * HammerheadEntity.this.rand.nextFloat();
				itementity.setMotion(
						(double) (0.3F * -MathHelper.sin(HammerheadEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(HammerheadEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.cos(f1) * f2),
						(double) (0.3F * MathHelper.sin(HammerheadEntity.this.rotationPitch * ((float) Math.PI / 180F))
								* 1.5F),
						(double) (0.3F * MathHelper.cos(HammerheadEntity.this.rotationYaw * ((float) Math.PI / 180F))
								* MathHelper.cos(HammerheadEntity.this.rotationPitch * ((float) Math.PI / 180F))
								+ MathHelper.sin(f1) * f2));
				HammerheadEntity.this.world.addEntity(itementity);
			}
		}
	}
	...
	static class SwimToTreasureGoal extends Goal {
		...
		SwimToTreasureGoal(HammerheadEntity dolphinIn) {
			this.dolphin = dolphinIn;
	...
	static class SwimWithPlayerGoal extends Goal {
		...
		SwimWithPlayerGoal(HammerheadEntity dolphinIn, double speedIn) {
			this.dolphin = dolphinIn;
			this.speed = speedIn;
			this.setMutexFlags(EnumSet.of(Goal.Flag.MOVE, Goal.Flag.LOOK));
		}
		 ...
		 */
		public boolean shouldExecute() {
			this.targetPlayer = this.dolphin.world.getClosestPlayer(HammerheadEntity.field_213810_bA, this.dolphin);
