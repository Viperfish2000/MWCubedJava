...
@OnlyIn(Dist.CLIENT)
public class GelLayer<T extends LivingEntity> extends LayerRenderer<T, SlimeModel<T>> {
   private final EntityModel<T> slimeModel = new SlimeModel<>(0);

   public GelLayer(IEntityRenderer<T, SlimeModel<T>> p_i50923_1_) {
      super(p_i50923_1_);
   }
   ...
   public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
      if (!entitylivingbaseIn.isInvisible()) {
         ...
         IVertexBuilder ivertexbuilder = bufferIn.getBuffer(RenderTypeMod.getTranslucentGlowing(this.getEntityTexture(entitylivingbaseIn), false));
